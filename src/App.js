...
const LockDetectionSystem = () => {
  // Position layouts for Boeing 777F
  ...
  // Random sensor changes disabled - manual control only via button
  ...
  const toggleSensor = () => {
    setSensorState(prev => {
      const newEngaged = !prev.engaged;
      
      // Log the change immediately
      const logEntry = {
        timestamp: new Date(),
        uldPosition: prev.uldPosition,
        lockPosition: lockPositions[prev.lockIndex],
        event: newEngaged ? 'ENGAGED' : 'DISENGAGED',
        value: newEngaged ? 1 : 0
      };
      setFlightLog(prevLog => [...prevLog, logEntry]);
      
      // Update ULD status immediately
      setUldStatuses(prevStatuses => {
        const updated = { ...prevStatuses };
        const targetULD = updated[prev.uldPosition];
        
        if (targetULD) {
          targetULD.locks[prev.lockIndex] = {
            ...targetULD.locks[prev.lockIndex],
            engaged: newEngaged,
            lastCheck: new Date()
          };
      ...
      return { ...prev, engaged: newEngaged };
    });
  };
